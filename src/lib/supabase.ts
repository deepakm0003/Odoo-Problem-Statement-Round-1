...
function createMockSupabaseClient() {
  ...
  const getMockItems = () => {
    ...
    },
    {
      ...
      },
      {
        id: '26',
        title: 'Sunglasses',
        description: 'Vintage-style sunglasses with gold frames. Perfect for sunny days.',
        category: 'accessories',
        size: 'One Size',
        condition: 'Good',
        point_value: 30,
        images: ['https://images.pexels.com/photos/6311463/pexels-photo-6311463.jpeg'],
        tags: ['sunglasses', 'vintage', 'gold'],
        user_id: 'demo-user-6',
      status: 'available',
      created_at: new Date().toISOString()
    }
  ...
  function createMockQueryBuilder(tableName: string, data: any[]) {
    ...
    const queryBuilder = {
      ...
      or: (query: string) => {
        // Handle OR queries like "title.ilike.%search%,description.ilike.%search%"
        const conditions = query.split(',');
        const searchValue = conditions[0].split('.').pop()?.replace(/%/g, '') || '';
        
        filteredData = filteredData.filter(item => {
          return conditions.some(condition => {
            const [field, operator] = condition.split('.');
            if (operator === 'ilike') {
  ...
  function seedSampleItems() {
    const items = [
      ...
      { id: 'item_accessories_6', title: 'Silver Bracelet', description: 'Shiny silver bracelet.', category: 'accessories', size: 'S', condition: 'Like New', images: ['https://images.pexels.com/photos/1484759/pexels-photo-1484759.jpeg'], tags: ['accessories', 'bracelet', 'silver'], point_value: 60, status: 'available', user_id: 'demo-user', created_at: new Date().toISOString(), updated_at: new Date().toISOString(), },
    ];
